...
int handle_request(int clientSocket, struct ParsedRequest *request, char *tempReq)
{
	...
	const char *h=(const char*)ParsedHeader_get(request,"Host");
	printf("Host :%s\n",h);
...
int main(int argc, char * argv[]) {
	...
	getsockname(client_socketId, (struct sockaddr*)&local_addr, &len);
	printf("Proxy socket: %s:%d\n", 
       inet_ntoa(local_addr.sin_addr), 
       ntohs(local_addr.sin_port));

	getpeername(client_socketId, (struct sockaddr*)&client_addr, &len);
	printf("Browser socket: %s:%d\n",
